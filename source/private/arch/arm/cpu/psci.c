...
INLINE uint16_t parse_fdt_fid(const void *fdt, const char *name, int psci_offset)
{
  ...
  if (prop == NULL) {
    pr_error(LOG_TAG, "'%s' was missing", name);
    return 0;
  } else {
...
static error_t parse_fdt(const void *fdt)
{
  ...
  if (psci < 0) {
    pr_error(LOG_TAG, "'psci' was missing");
    return ERR_MALFORMED;
  }

  ctx.compatible = fdt_compatible(fdt, psci, matches);
  if (ctx.compatible == NULL) {
    pr_error(LOG_TAG, "incompatible PSCI version");
    return ERR_NOTSUPPORTED;
  }
  pr_info(LOG_TAG, "compatible: %s", ctx.compatible);
  ...
  if (method == NULL || strcmp(method, "smc") != 0) {
    pr_error(LOG_TAG, "this is hypervisor so method must be 'smc'");
    return ERR_NOTSUPPORTED;
  }
  return ERR_NONE;
}
...
static error_t init_0_2()
{
  ...
  if (ver < PSCI_RET_NOT_SUPPORTED) {
    pr_error(LOG_TAG, "firmware does not comply with the v0.2 spec");
    return ERR_NOTSUPPORTED;
  }
  const int major = PSCI_VERSION_MAJOR(ver);
  const int minor = PSCI_VERSION_MINOR(ver);
  pr_info(LOG_TAG, "firmware version: v%d.%d", major, minor);
  if (major == 0 && minor < 2) {
    pr_error(LOG_TAG, "conflicting PSCI version detected");
    return ERR_INVALID;
  }
  return ERR_NONE;
}
...
static error_t psci_init(const void *fdt)
{
  ...
  pr_info(LOG_TAG, "fid_cpu_on:  0x%x", ctx.fid_cpu_on);
  pr_info(LOG_TAG, "fid_cpu_off: 0x%x", ctx.fid_cpu_off);
  return ec;
}

static error_t psci_cpu_on(int idx, uint64_t entry_point)
{
  if (ctx.fid_cpu_on == 0) return ERR_NOTSUPPORTED;
...
{
  if (ctx.fid_cpu_off == 0) return ERR_NOTSUPPORTED;
